...
{
  boost::mutex::scoped_lock lock(range_message_mutex_);   // This a threading operation and used to lock some data some time for specific work and can unlock once done
  range_msgs_buffer_.push_back(*range_message);          //This is a list, it will stores the all range_message format data from backwards 
}

void RangeSensorLayer::updateCostmap()
{
  std::list<sensor_msgs::Range> range_msgs_buffer_copy;    //Creating a list to store Range msg format. Ex: std::list<int> l = { 7, 5, 16, 8 };

  range_message_mutex_.lock();                             // locking thread for variable
  range_msgs_buffer_copy = std::list<sensor_msgs::Range>(range_msgs_buffer_);   //Assigning Range sensor msg list  
  range_msgs_buffer_.clear();                             // clearing thread 
  range_message_mutex_.unlock();                          // Relesing thread 

  // iterating the Range msgs
  ...
  {
    processRangeMessageFunc_(*range_msgs_it);   //Binds the range msgs
  }
}

void RangeSensorLayer::processRangeMsg(sensor_msgs::Range& range_message)   
{
  // std::cout << range_message.min_range <<std::endl;
  if (range_message.min_range == range_message.max_range)      //This condition for fixed distance applns ie, min_range == max_range
...
void RangeSensorLayer::processFixedRangeMsg(sensor_msgs::Range& range_message)
{
  ...
  updateCostmap(range_message, clear_sensor_cone);    //This calls updateCostmap
}
...
//Main Functions to perform Costmap Updates related activities
void RangeSensorLayer::updateCostmap(sensor_msgs::Range& range_message, bool clear_sensor_cone)
{
  max_angle_ = range_message.field_of_view / 2;      // 25/2 = 12.5 degrees  Note: To increase FOV and view long straight obstacle layer remove '/2'

  geometry_msgs::PointStamped in, out;               // Point x,y,z Message type points created
  in.header.stamp = range_message.header.stamp;      //Same range sensor publishing timestamp
  in.header.frame_id = range_message.header.frame_id;  //Same range frame id ie, base_laser_link

  if (!tf_->canTransform(global_frame_, in.header.frame_id, in.header.stamp, ros::Duration(0.1)))
  {
    // std::cout << global_frame_ <<std::endl;
    // You might get this error whenever if are failed tranform tf info between odom and base_laser_link of sensor frame(If it giving occasionally then neglect this error)
  ...
  tf_->transform(in, out, global_frame_);   //Here the 'in' PointStamped msg info directly converted to 'out' msgs format using global_frame timestamp info

  double ox = out.point.x, oy = out.point.y;  //Assign the out.x(ie, converted 'in' msg info) to ox   Initially in.point.x =0= out.point.x

  in.point.x = range_message.range;      //Defining 'in' point x position

  tf_->transform(in, out, global_frame_);  //Here again convertion 'in' data to 'out' data, so here in.point.x = 1 =out.point.x

  double tx = out.point.x, ty = out.point.y;   //Assign updated value to tx = 1, ty=0

  // calculate target props, Here calculating x,y,theta and dist information between previous(ox) and current_point(tx) this info draw layer on costmap. If distance of previous 'out' pt and current 'out' are more then it will draw rectangle width like straight line otherwise it indicates as point
  double dx = tx - ox;   
  ...
  worldToMapNoBounds(ox, oy, Ox, Oy);   //Converting the ox, oy location info inside the map into Ox,Oy(origin)
  bx1 = bx0 = Ox;
  by1 = by0 = Oy;
  touch(ox, oy, &min_x_, &min_y_, &max_x_, &max_y_);    //Updates the bounding box on costmap based on parameter given 

  // Update Map with Target Point
  unsigned int aa, ab;
  if (worldToMap(tx, ty, aa, ab))        // This converts the world co-ordinate points(tx,ty) into Map co-ordinates point(aa, ab)
  {
    setCost(aa, ab, LETHAL_OBSTACLE);                //This will set aa, ab cells inside the map to 233 cost value(dark cells)
  ...
  mx = ox + cos(theta - max_angle_) * d * 1.2;    //calculating the left side of cone by using origin(middle) point of cone with sonar FOV
  my = oy + sin(theta - max_angle_) * d * 1.2;
  worldToMapNoBounds(mx, my, Ax, Ay);             //transforming let point to map coordinate point
  bx0 = std::min(bx0, Ax);
  bx1 = std::max(bx1, Ax);
  by0 = std::min(by0, Ay);
  by1 = std::max(by1, Ay);
  ...
  mx = ox + cos(theta + max_angle_) * d * 1.2;     //calculating right point
  my = oy + sin(theta + max_angle_) * d * 1.2;
